...
REGISTRY ?= quay.io
REPOSITORY ?= $(REGISTRY)/redhat-cop/operator-utils
...
export GITHUB_PAGES_REPO ?= redhat-cop/operator-utils
export HELM_CHARTS_SOURCE ?= charts
export HELM_CHART_DEST ?= $(GITHUB_PAGES_DIR)

LDFLAGS := "-X github.com/redhat-cop/operator-utils/version.Version=$(VERSION) \
	-X github.com/redhat-cop/operator-utils/version.Vcs=$(BUILD_COMMIT) \
	-X github.com/redhat-cop/operator-utils/version.Timestamp=$(BUILD_TIMESTAMP) \
	-X github.com/redhat-cop/operator-utils/version.Hostname=$(BUILD_HOSTNAME)"
...
manager: generate fmt vet
	go build -o build/_output/bin/operator-utils  -ldflags $(LDFLAGS) github.com/redhat-cop/operator-utils/cmd/manager

# Build manager binary
manager-osx: generate fmt vet
	GOOS=darwin go build -o build/_output/bin/operator-utils -ldflags $(LDFLAGS) github.com/redhat-cop/operator-utils/cmd/manager
