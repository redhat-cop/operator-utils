...
func GetLockedPatchedFromLockedPatchesSet(lockedPatchSet *Set, hashableLockedPatchMap map[lockedpatch.HashableLockedPatch]lockedpatch.LockedPatch) []lockedpatch.LockedPatch {
	lockedPatches := []lockedpatch.LockedPatch{}
	for _, hashableLockedPatch := range lockedPatchSet.List() {
		lockedPatches = append(lockedPatches, hashableLockedPatchMap[hashableLockedPatch])
	}
...
var (
	...
	keyExists   = struct{}{}
	nonExistent lockedpatch.HashableLockedPatch
)

// Set is the main set structure that holds all the data
// and methods used to working with the set.
type Set struct {
	m map[lockedpatch.HashableLockedPatch]struct{}
}

// New creates and initializes a new Set.
func New(ts ...lockedpatch.HashableLockedPatch) *Set {
...
func NewWithSize(size int) *Set {
	return &Set{make(map[lockedpatch.HashableLockedPatch]struct{}, size)}
}

// Add includes the specified items (one or more) to the Set. The underlying
// Set s is modified. If passed nothing it silently returns.
func (s *Set) Add(items ...lockedpatch.HashableLockedPatch) {
...
func (s *Set) Remove(items ...lockedpatch.HashableLockedPatch) {
	for _, item := range items {
		delete(s.m, item)
	}
}
...
func (s *Set) Pop() lockedpatch.HashableLockedPatch {
	for item := range s.m {
		delete(s.m, item)
		return item
	}
...
// returned.
func (s *Set) Pop2() (lockedpatch.HashableLockedPatch, bool) {
	for item := range s.m {
		delete(s.m, item)
		return item, true
	}
...
func (s *Set) Has(items ...lockedpatch.HashableLockedPatch) bool {
	has := false
	for _, item := range items {
...
func (s *Set) HasAny(items ...lockedpatch.HashableLockedPatch) bool {
	has := false
	for _, item := range items {
		if _, has = s.m[item]; has {
			break
...
func (s *Set) Clear() {
	s.m = make(map[lockedpatch.HashableLockedPatch]struct{})
}
...
func (s *Set) IsEqual(t *Set) bool {
	...
	equal := true
	t.Each(func(item lockedpatch.HashableLockedPatch) bool {
		_, equal = s.m[item]
...
func (s *Set) Each(f func(item lockedpatch.HashableLockedPatch) bool) {
	for item := range s.m {
		if !f(item) {
			break
		}
...
func (s *Set) List() []lockedpatch.HashableLockedPatch {
	v := make([]lockedpatch.HashableLockedPatch, 0, s.Size())
	for item := range s.m {
		v = append(v, item)
	}
	return v
