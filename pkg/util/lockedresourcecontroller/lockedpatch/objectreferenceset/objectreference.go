...
func NewWithSize(size int) *Set {
	return &Set{make(map[corev1.ObjectReference]struct{}, size)}
}

// Add includes the specified items (one or more) to the Set. The underlying
// Set s is modified. If passed nothing it silently returns.
func (s *Set) Add(items ...corev1.ObjectReference) {
...
func (s *Set) Remove(items ...corev1.ObjectReference) {
	for _, item := range items {
		delete(s.m, item)
	}
}
...
func (s *Set) Pop() corev1.ObjectReference {
	for item := range s.m {
		delete(s.m, item)
		return item
	}
...
// returned.
func (s *Set) Pop2() (corev1.ObjectReference, bool) {
	for item := range s.m {
		delete(s.m, item)
		return item, true
	}
...
func (s *Set) Has(items ...corev1.ObjectReference) bool {
	has := false
	for _, item := range items {
...
func (s *Set) HasAny(items ...corev1.ObjectReference) bool {
	has := false
	for _, item := range items {
		if _, has = s.m[item]; has {
			break
...
func (s *Set) Clear() {
	s.m = make(map[corev1.ObjectReference]struct{})
}
...
func (s *Set) IsEqual(t *Set) bool {
	...
	equal := true
	t.Each(func(item corev1.ObjectReference) bool {
		_, equal = s.m[item]
...
func (s *Set) Each(f func(item corev1.ObjectReference) bool) {
	for item := range s.m {
		if !f(item) {
			break
		}
...
func (s *Set) List() []corev1.ObjectReference {
	v := make([]corev1.ObjectReference, 0, s.Size())
	for item := range s.m {
		v = append(v, item)
	}
	return v
